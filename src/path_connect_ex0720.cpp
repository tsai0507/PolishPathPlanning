...
void SectPath::show()
{
    ...
    viewer.addPointCloud<pcl::PointXYZRGB>(cloud, "sample_cloud");
...
void SectPath::OnePath(Eigen::Vector3f plane_point)
{
    ...
    // std::shared_ptr<Spline> foo = std::make_shared<Spline>(node_number, point_y, point_x, point_z);
    delete[] point_x;
    delete[] point_y;
    delete[] point_z;
    ...
    *cloud += *other_cloud;
    ...
    for(auto onePathXYZ: WayPointsXYZList){
        for(auto PointXYZ: onePathXYZ){
            ...
            //get normal cloud using average normal
            kdtree.nearestKSearch(SPoint, 10, id, dis);
            Eigen::Vector3f vecSum(0, 0, 0);
            for(auto i: id){
                auto nor = normal_cloud->points[id[0]];
                Eigen::Vector3f vec(nor.normal_x, nor.normal_y, nor.normal_z);
                vecSum += vec;
            }
            pcl::Normal N(vecSum.x()/id.size(), vecSum.y()/id.size(), vecSum.z()/id.size());
            // std::cout << N << std::endl;
            // Eigen::Vector3f normal(float(-N.normal_x), float(-N.normal_y), float(-N.normal_z));
            // Eigen::Quaternionf rotation;
            // rotation.setFromTwoVectors(Eigen::Vector3f::UnitZ(), normal);
            // Eigen::Matrix3f rotationMatrix = rotation.toRotationMatrix();

            Eigen::Vector3f Approach(float(-N.normal_x), float(-N.normal_y), float(-N.normal_z));
            // Eigen::Vector3f Approach(0, 0, 1);
